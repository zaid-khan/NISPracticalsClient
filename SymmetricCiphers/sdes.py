...
def SDESEncrypt(key, plaintext):
    """Encrypt plaintext with given key WRAPPER"""
    keyvalue = int(key, 2)
    plaintextvalue = ord(plaintext)
    encryptedtext = encrypt(keyvalue, plaintextvalue)
    return chr(encryptedtext)
